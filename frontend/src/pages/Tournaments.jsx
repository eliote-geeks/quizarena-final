  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="relative max-w-[1400px] mx-auto px-6 lg:px-10 py-12">
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="font-arcade text-xl text-white/50">{tr.startsIn}</div>
                </div>
                <h3 className="font-display font-bold uppercase text-xl tracking-tight mb-4 text-white">
                  {tr.name[lang]}
                </h3>
                <div className="grid grid-cols-3 gap-2 text-xs">
                  <div>
                    <div className="text-slate-500 uppercase tracking-widest text-[10px]">{t.tournament.entryFee}</div>
                    <div className="font-arcade text-base" style={{ color: "#E5A800" }}>{tr.entryFee}</div>
                  </div>
                  <div>
                    <div className="text-slate-500 uppercase tracking-widest text-[10px]">{t.tournament.slots}</div>
                    <div className="font-arcade text-base text-white">{tr.registered}/{tr.slots}</div>
                  </div>
                  <div>
                    <div className="text-slate-500 uppercase tracking-widest text-[10px]">{t.common.prizePool}</div>
                    <div className="font-arcade text-base" style={{ color: "#E5A800" }}>{tr.prizePool.toLocaleString()}</div>
                  </div>
                </div>
              </motion.button>
            );
        ...
        <div className="rounded-2xl border-2 bg-[#0B0B14] p-6 lg:p-8" style={{ borderColor: "rgba(229,168,0,0.4)" }}>
          <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
            <div>
              <div className="text-xs uppercase tracking-widest text-slate-500 mb-1">Tournament</div>
              <h2 className="font-display font-black uppercase text-3xl tracking-tighter" style={{ color: "#E5A800" }}>
                {current.name[lang]}
              </h2>
            </div>
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#12121E] border border-white/10">
                ...
                <span className="font-arcade text-xl" style={{ color: "#E5A800" }}>{current.prizePool.toLocaleString()}</span>
              </div>
              <button
                onClick={handleRegister}
                data-testid="tournament-register-btn"
                className="px-6 py-3 text-black font-bold uppercase tracking-widest rounded-md hover:shadow-[0_0_24px_rgba(229,168,0,0.5)] transition flex items-center gap-2"
            ...
            <div className="flex gap-8 min-w-max">
              {BRACKET.rounds.map((round, ri) => (
                <div key={round.label} className="flex flex-col justify-around min-w-[230px]">
                  <div className="text-[10px] uppercase tracking-widest mb-3 font-bold" style={{ color: "#E5A800" }}>
                    {t.tournament[round.label]}
                  </div>
                  <div className={`flex flex-col gap-${ri === 3 ? "0" : (ri + 1) * 4}`} style={{ gap: `${(ri + 1) * 16}px` }}>
                    {round.matches.map((m, mi) => (
                      <MatchNode key={mi} match={m} accent="#E5A800" isFinal={ri === 3} />
                    ))}
                  </div>
                </div>
              ))}
...
function MatchNode({ match, accent, isFinal }) {
  return (
    <div
      ...
      style={{ borderColor: isFinal ? accent : "rgba(255,255,255,0.08)", boxShadow: isFinal ? `0 0 24px ${accent}50` : "none" }}
    >
      {isFinal && (
        <div className="text-black text-[10px] font-bold uppercase tracking-widest text-center py-1 flex items-center justify-center gap-1" style={{ background: accent }}>
